        ...
        for file in file_list:
            if os.path.isfile(file):
                with open(file, 'r', encoding='utf-8') as f:
                    fdata = yaml.load(f.read(), Loader=self._build_yaml_loader()) or {}
                    base_model_config = fdata.get('base_model_config')
                    if base_model_config:
                        base_file = base_model_config
                        if not os.path.isabs(base_file):
                            base_file = os.path.join(os.path.dirname(file), base_file)
                        if os.path.isfile(base_file):
                            with open(base_file, 'r', encoding='utf-8') as bf:
                                bdata = yaml.load(bf.read(), Loader=self._build_yaml_loader()) or {}
                                if bdata.get('hyper_parameters'):
                                    hyper_parameters.extend(bdata['hyper_parameters'])
                                file_config_dict.update(bdata)
