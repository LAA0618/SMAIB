...
def get_model(model_name):
    ...
    if not importlib.util.find_spec(module_path, __name__):
        raise ValueError(
            f"Unknown model '{model_name}'. Expected a module at '{module_path}'."
        )
    model_module = importlib.import_module(module_path, __name__)

    if not hasattr(model_module, model_name):
        raise ValueError(
            f"Module '{module_path}' does not define model class '{model_name}'."
        )
