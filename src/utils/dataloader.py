...
class AbstractDataLoader(object):
    ...
    def _filter_cold_start_eval_users(self, eval_users):
        max_interactions = self._config_int('cold_start_max_interactions', 5)
        uid_field = self.additional_dataset.uid_field
        iid_field = self.additional_dataset.iid_field
        train_counts = self.additional_dataset.df.groupby(uid_field)[iid_field].size()
        cold_users = set(train_counts[train_counts <= max_interactions].index)
        before = len(eval_users)
        filtered = np.asarray([u for u in eval_users if u in cold_users])
        self.logger.info(
            'Paper-style cold-start user evaluation enabled: keep %d/%d users with train interactions <= %d',
            len(filtered), before, max_interactions
        )
        return filtered
    @property
    def pr_end(self):
...
class EvalDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, additional_dataset=None,
        ...
        if self._config_bool('cold_start_users', False):
            self.eval_u = self._filter_cold_start_eval_users(self.eval_u)
